...
- Text box accepts a minimum of 5,000 characters
...
- Character count is visible when input exceeds 4,000 characters
...
- The analyst's mode selection is logged in the audit trail alongside the query
...
*Acceptance criteria:*
- After query submission (before pipeline starts), a dataset selection screen shows top 5–8 suggested files ranked by semantic relevance to the query
- Each suggestion displays: file name, file type icon, upload date, and a one-line description generated by the Analyzer
...
- If the analyst has no uploaded files and no institutional datasets are available for their department, a clear message is shown explaining the situation before submission
...
Core regulatory datasets are loaded by IT/data engineering via backend pipelines — not through any UI. Analysts can supplement with personal file uploads. Upload capability is ON by default with an admin toggle to disable.

*Acceptance criteria:*
- Institutionally loaded datasets appear in the analyst's dataset selection screen alongside their personal uploads, clearly labelled as "Institutional" vs. "Personal upload"
- Institutional datasets cannot be deleted by analysts
...
- Given a task ID, an admin can retrieve the exact data snapshot used in that analysis
- Updating a file in the personal workspace (re-uploading a newer version) does not modify snapshots linked to past analyses
- The snapshot storage is included in the audit log by reference (snapshot ID), not as a full data copy in the log itself

---

**FR-DA-05 — Access control**
V1 uses department-level RBAC. Each analyst sees datasets belonging to their department only.

*Acceptance criteria:*
- An analyst assigned to Department A cannot see, select, or query datasets assigned to Department B
...
- Admin can reassign a dataset's department via the admin panel; the change takes effect within 60 seconds
